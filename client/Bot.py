...
class AudioToRedisTrack(AudioStreamTrack):
    def __init__(self, source_track, redis_client, roomId):
        super().__init__()
...
class Demo:
    def __init__(self, roomId, player=None, recorder=MediaBlackhole(), loop=None):
        if loop is None:
            try:
    ...
    async def run(self):
        await self._websocket.connect(self._uri)
...
#        print('ice_server',ice_server)
    ...
    async def consume(self, id, producerId, kind, rtpParameters, appData):
        ...
        #Bővítésem
        if kind == "audio":
            redis_audio_track = AudioToRedisTrack(consumer.track, self.redis, self.roomId)
            self._recorder.addTrack(redis_audio_track)
    ...
    async def close(self):
        ...
        self._closed = True
        print(f"Stopping bot for room: {self.roomId}")

        # 1. Stop the recorder first (this stops the tracks)
        if self._recorder:
            try:
                await self._recorder.stop()
        ...
        # 2. Close consumers and producers
        ...
        print("SHUTDOWN")
    def build_uri(self)   :
        return f"wss://test.remai.hu/?peerId=python&roomId={self.roomId}"
